...
async function generatePost(apiKey, platform, tone, topic, extraContext = "") {
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1000,
      system: `You are a world-class SaaS growth marketer specializing in fitness technology. You write social media posts for FlowFit — an AI-powered fitness SaaS platform that generates personalized workout plans, tracks progress, and adapts to each user's goals. The platform offers a free trial. Write posts that are genuine, punchy, and drive conversions. Avoid clichés. Sound human. Use platform-specific best practices. Return ONLY the post text — no preamble, no quotes, no labels, no markdown backticks.`,
      messages: [{
        role: "user",
        content: `Write a ${tone.toLowerCase()} ${platformInfo.label} post about: "${topic}". Max length: ${platformInfo.limit} characters. End with these hashtags on a new line: ${platformInfo.tag}${extraContext ? `. Extra context: ${extraContext}` : ""}. Return ONLY the post text.`,
      }],
    }),
  });
  if (!response.ok) {
      ...
      <main style={S.main}>
        ...
        {!keyLocked && (
          <div style={S.keyBanner}>
            ...
            <button style={S.keyBtn} onClick={() => { if (apiKey.startsWith("sk-")) setKeyLocked(true); }}>
              Save
            </button>
            <a href="https://console.anthropic.com/keys" target="_blank" rel="noreferrer" style={S.keyLink}>
              Get key →
            </a>
          </div>
        )}
