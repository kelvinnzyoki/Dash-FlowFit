  ...
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are a world-class SaaS growth marketer specializing in fitness technology. You write social media posts for FlowFit — an AI-powered fitness SaaS platform that generates personalized workout plans, tracks progress, and adapts to each user's goals. The platform offers a free trial. Write posts that are genuine, punchy, and drive conversions. Avoid clichés. Sound human. Use platform-specific best practices. Return ONLY the post text — no preamble, no quotes, no labels, no markdown backticks.",
        },
        {
          role: "user",
          content: userPrompt,
...
export default function App() {
  ...
  const envKey = import.meta.env.GROQ_API_KEY || import.meta.env.VITE_GROQ_API_KEY || "";
  ...
  const handleGenerate = async () => {
    if (!apiKey) { setGenError("Enter your Groq API key first."); return; }
      ...
      <main style={S.main}>
        ...
        {!keyLocked && (
          <div style={S.keyBanner}>
            <span style={S.keyLabel}>🔑 Groq API Key</span>
            <input type="password" placeholder="gsk_..." value={apiKey}
              onChange={e => setApiKey(e.target.value)} style={S.keyInput} />
            <button style={S.keyBtn} onClick={() => { if (apiKey.startsWith("gsk_")) setKeyLocked(true); }}>
              Save
            </button>
            <a href="https://console.groq.com/keys" target="_blank" rel="noreferrer" style={S.keyLink}>
              Get key →
            </a>
          </div>
        )}
